...
def _reorder_for_sdmetrics(
    ...
    # Validator records store categorical legal values as strings, while real CSV
...
class TabDiffSelectionEvaluator:
    def __init__(
        self,
        dataset_name: str,
        device: str = "cpu",
    ...
    def _complete_y_only_data(
        ...
        completed = deepcopy(real_data)
        completed[target_col_idx] = syn_target_col
        return completed

    def _prepare_density_frames(
        self,
        syn_data: pd.DataFrame,
        ...
        if y_only:
            target_col_idx = int(info["target_col_idx"][0])
            syn_data = self._complete_y_only_data(syn_data, real_data, target_col_idx)

        new_real_data, new_syn_data, metadata = _reorder_for_sdmetrics(real_data, syn_data, info)
        return new_real_data, new_syn_data, metadata, y_only

    def _evaluate_density(self, syn_data: pd.DataFrame) -> tuple[dict[str, Any], dict[str, Any]]:
        from sdmetrics.reports.single_table import DiagnosticReport, QualityReport

        new_real_data, new_syn_data, metadata, y_only = self._prepare_density_frames(syn_data)
    ...
    def _evaluate_c2st(self, syn_data: pd.DataFrame) -> tuple[dict[str, Any], dict[str, Any]]:
        from sdmetrics.single_table import LogisticDetection

        new_real_data, new_syn_data, metadata, _ = self._prepare_density_frames(syn_data)
