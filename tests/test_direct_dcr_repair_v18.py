...
from post_selection_tool.direct_dcr_repair_v18 import apply_direct_dcr_repair_v18
...
def test_direct_dcr_repair_v18_uses_tight_target_duplicate_fill() -> None:
    ...
    _, final_records, report = apply_direct_dcr_repair_v18(
    ...
    assert report["version"] == "direct_dcr_repair_v18"
    assert report["v18_source_strategy"] == "direct_dcr_repair_v15"
    assert report["candidate_full_eval_used"] is False
    assert report["intermediate_candidate_count"] == 0
    assert report["v18_tight_target_margin"] == 0.048
    assert report["duplicate_fill_pair_count"] > 0
    assert report["selected_swaps"] == report["desired_swaps"]
    assert report["final_dcr_estimate"] <= report["target_dcr"] + (1.0 / len(final_records))
