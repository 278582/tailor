...
from post_selection_tool.direct_dcr_repair_v16 import apply_direct_dcr_repair_v16
...
def test_direct_dcr_repair_v16_jitters_duplicate_fill_rows() -> None:
    ...
    _, final_records, report = apply_direct_dcr_repair_v16(
        ...
        duplicate_jitter_scale=1e-3,
    )

    final_row_keys = [(record["row"]["x"], record["row"]["target"]) for record in final_records]
    assert report["applied"] is True
    assert report["version"] == "direct_dcr_repair_v16"
    ...
    assert report["duplicate_jitter_rows"] > 0
    assert report["duplicate_jitter_unique_rows_after"] > report["duplicate_jitter_unique_rows_before"]
    assert len(set(final_row_keys)) == len(final_row_keys)
