...
from post_selection_tool.direct_dcr_repair_v17 import apply_direct_dcr_repair_v17
...
def test_direct_dcr_repair_v17_caps_duplicate_fill_reuse() -> None:
    ...
    _, final_records, report = apply_direct_dcr_repair_v17(
        ...
        duplicate_fill_max_reuse_per_add=2,
        duplicate_fill_relax_factor=2,
        duplicate_fill_max_relax_rounds=3,
        duplicate_fill_unbounded_fallback=False,
    )

    final_ids = [record["candidate_id"] for record in final_records]
    assert report["applied"] is True
    assert report["version"] == "direct_dcr_repair_v17"
    assert report["candidate_full_eval_used"] is False
    assert report["intermediate_candidate_count"] == 0
    assert report["pair_builder_mode"] == "target_then_limited_generic_reuse_capped_duplicate_fill"
    assert report["unique_pair_count_before_duplicate_fill"] < report["desired_swaps"]
    assert report["duplicate_fill_pair_count"] > 0
    assert report["duplicate_fill_reuse_capped"] is True
    assert report["duplicate_fill_relaxed_cap_used"] == 2
    assert report["duplicate_fill_max_reuse_count"] <= 2
