...
from post_selection_tool.direct_dcr_repair_v14 import apply_direct_dcr_repair_v14
...
def test_direct_dcr_repair_v14_forces_bounded_signal_for_small_inputs() -> None:
    ...
    extra_test_same_target = [_record(20 + idx, 100.3 + idx * 0.01, "0") for idx in range(8)]
    extra_test_cross_target = [_record(28 + idx, 100.3 + idx * 0.01, "1") for idx in range(20)]
    extra_real = [_record(48 + idx, 0.3 + idx * 0.01, "0") for idx in range(8)]
    ...
    _, final_records, report = apply_direct_dcr_repair_v14(
        ...
        signal_device="cpu",
        report_id_limit=3,
        generic_remove_budget=16,
    )

    final_ids = {record["candidate_id"] for record in final_records}
    assert report["applied"] is True
    assert report["version"] == "direct_dcr_repair_v14"
    assert report["bounded_mode"] is True
    assert report["base_strategy"] == "full_reference_force_bounded_target_then_limited_generic_fill_v14"
    assert report["force_bounded_small_mode"] is True
