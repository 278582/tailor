...
from post_selection_tool.direct_dcr_repair_v21 import apply_direct_dcr_repair_v21
...
def test_direct_dcr_repair_v21_expands_swap_cap() -> None:
    ...
    extra_test = [_record(20 + idx, 100.3 + idx * 0.01) for idx in range(5)]
    extra_real = [_record(25 + idx, 0.3 + idx * 0.01) for idx in range(8)]
    ...
    _, final_records, report = apply_direct_dcr_repair_v21(
        ...
        large_candidate_rows=len(pool_records),
        large_reference_rows=0,
        large_max_swaps=8,
        large_candidate_neighbors=8,
    ...
    assert report["version"] == "direct_dcr_repair_v21"
    assert report["v21_source_strategy"] == "direct_dcr_repair_v15"
    assert report["candidate_full_eval_used"] is False
    assert report["intermediate_candidate_count"] == 0
    assert report["v21_center_target_margin"] == 0.03
    assert report["v21_max_swap_fraction"] == 0.35
    assert report["max_swaps"] == 7
    assert report["selected_swaps"] == min(report["desired_swaps"], 7)
