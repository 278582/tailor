...
from post_selection_tool.direct_dcr_repair_v20 import apply_direct_dcr_repair_v20
...
def test_direct_dcr_repair_v20_uses_near_center_target_duplicate_fill() -> None:
    ...
    _, final_records, report = apply_direct_dcr_repair_v20(
    ...
    assert report["version"] == "direct_dcr_repair_v20"
    assert report["v20_source_strategy"] == "direct_dcr_repair_v15"
    assert report["candidate_full_eval_used"] is False
    assert report["intermediate_candidate_count"] == 0
    assert report["v20_near_center_target_margin"] == 0.02
