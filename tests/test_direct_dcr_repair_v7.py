...
from post_selection_tool.direct_dcr_repair_v7 import apply_direct_dcr_repair_v7
...
def test_direct_dcr_repair_v7_bounded_large_mode_caps_work_and_report_ids() -> None:
    ...
    _, final_records, report = apply_direct_dcr_repair_v7(
        ...
        large_selected_signal_rows=8,
        large_candidate_rows=8,
        large_reference_rows=2,
        large_max_swaps=5,
    ...
    assert report["version"] == "direct_dcr_repair_v7"
    assert report["bounded_mode"] is True
    assert report["sampled_estimate"] is True
